...
Running the `brew tap mongodb/brew` command will add the Homebrew repository for MongoDB, followed by the `brew install mongodb-community` command to install MongoDB Community Edition community edition is installed by running the command `brew install mongodb-community`. This installation includes the following binaries

- `mongod` server
- `mongos` sharding cluster query router
